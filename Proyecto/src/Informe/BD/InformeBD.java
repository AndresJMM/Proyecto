...
public class InformeBD {
    ...
    public static boolean informeRealizado(String d){
        ...
        try {
            ...
            PreparedStatement cs = conn.prepareCall("SELECT CONTENIDO FROM INFORMES WHERE to_char(FECHA,'MM-YYYY') = ?");
            cs.setString(1, d);
    ...
    public static void almacenarInforme(String d, String informe){
        ...
        try {
            ...
            if(informeRealizado(d)){
                PreparedStatement cs = conn.prepareCall("UPDATE INFORMES SET FECHA=?, CONTENIDO=? WHERE to_char(FECHA,'MM-YYYY') = ?");
                ...
                PreparedStatement cs = conn.prepareCall("INSERT INTO INFORMES VALUES(?,?)");
                cs.setDate(1, sqlDate);
                cs.setString(2, informe);
                cs.executeUpdate();
                try{
